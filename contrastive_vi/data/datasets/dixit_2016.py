...
def download_dixit_2016(output_path: str) -> None:
    """
    Download Dixit et al. 2016 data from the hosting URL and extracts the files.
...
def _read_perturbseq_df(directory: str, file_prefix: str) -> pd.DataFrame:
    ...
    counts = sc.read_mtx(
        os.path.join(directory, f"{file_prefix}.mtx.txt.gz")
    ).T.X.todense()
...
def _read_perturbation_information(directory: str, file_prefix: str) -> pd.DataFrame:
    ...
    perturb_df = pd.DataFrame(
        perturb_dict_concat.values(),
        perturb_dict_concat.keys(),
...
def preprocess_dixit_2016(
    download_path: str, n_top_genes: int, normalization_method: str = "tc"
) -> AnnData:
    """
    Preprocess expression data from Dixit et al., 2016.

    Args:
    ----
        download_path: Path containing the downloaded Dixit et al. 2016 data files.
    ...
    control_perturbation_df = _read_perturbation_information(
        download_path, "GSM2396857_dc_0hr_cbc_gbc_dict.csv.gz"
    )
    stim_perturbation_df = _read_perturbation_information(
        download_path, "GSM2396856_dc_3hr_cbc_gbc_dict_strict.csv.gz"
    )
    ...
    merged.obs["perturbation_simple"] = [
        "Multi" if len(x.split()) > 1 else x for x in merged.obs.perturbation
    ]
    merged.obs["perturbation_gene"] = [
        x.split("_")[1] if "_" in x else "None"
        for x in merged.obs["perturbation_simple"]
    ]
    merged.obs["MOI"] = [
        len(x.split()) if x != "None" else 0 for x in merged.obs.perturbation
    ]
    merged = preprocess_workflow(
        adata=merged, n_top_genes=n_top_genes, normalization_method=normalization_method
    )
    return merged
